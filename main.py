...
if st.button('Calculate'):
    ...
    if potential == 'Rydberg Potential' :
        ...
        st.write('OBS: downloading this file will reload the whole page!')
        st.subheader("Graph of Energy(eV) per Distance($\AA$)")
        ...
        fig1.update_layout(
            xaxis_title='U(eV)',  # Replace 'X Axis Label' with your desired X-axis label
            yaxis_title=r"$r(\AA)$"   # Replace 'Y Axis Label' with your desired Y-axis label
                    )
        ...
        if L_uploaded_file is not None:
            ...
            popt, pcov = curve_fit(ILJ, rL, UL, p0,maxfev=5000)
            alpha,beta,mp, De, Req = popt
        ...
        st.plotly_chart(fig1, use_container_width=True, title = 'test')
